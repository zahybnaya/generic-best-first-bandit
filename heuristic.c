...
double h1(int board[2][NUM_PITS+1], int side, int dummy) {
  double val = board[max][store] - board[min][store];
  double die;
  double noise;
...
double h2(int board[2][NUM_PITS+1], int side, int dummy) {
  ...
  score = board[max][store]-board[min][store];
  for (i = 1; i < NUM_PITS+1; i++)
    score = score + board[min][i] - board[max][i];
...
double h6(int board[2][NUM_PITS+1], int side, int numPlayouts) {
  ...
  for (i = 0; i < numPlayouts; i++)
    playoutSum += finerPlayout(board, side);
