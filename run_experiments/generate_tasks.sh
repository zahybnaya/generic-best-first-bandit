	...
	do 
		...
		do 
			...
			do 
				...
				do 
				  CMD="${EXECUTABLE} ${DOMAIN} b u -h1 ${c_H} -h2 ${c_H} -c1 ${c_C} -c2 ${c_C} -i1 ${c_ITERATIONS} -i2 ${c_ITERATIONS} -s ${SEED} -g ${NUM_GAMES} -ts1 ${TYPE_SYS} -t1 ${c_SIZE_OF_STS} -p1 ${c_P}"
				 OUT_FILE="bfbVSuct_${c_H}_${c_C}_${c_P}_${c_ITERATIONS}_${SEED}_${NUM_GAMES}_${TYPE_SYS}_${c_SIZE_OF_STS}.csv"
				  if $VERBOSE ; then 
					echo "${CMD} > ${OUT_FILE}" 
				  fi
				  #eval ${CMD} > ${OUT_FILE}
	...
	do 
		...
		do 
			...
			do 
				for c_P in ${BANDIT_POLICY[@]}
				do
				  CMD="${EXECUTABLE} ${DOMAIN} b m -h1 ${c_H} -h2 ${c_H} -c1 ${c_C} -i1 ${c_ITERATIONS} -s ${SEED} -g ${NUM_GAMES} -ts1 ${TYPE_SYS} -t1 ${c_SIZE_OF_STS} -p1 ${c_P}"
			...
			CMD="${EXECUTABLE} ${DOMAIN} u m -h1 ${c_H} -h2 ${c_H} -c1 ${c_C} -i1 ${c_ITERATIONS} -s ${SEED} -g ${NUM_GAMES}"
